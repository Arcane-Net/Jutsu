...
public abstract class EarthAbility extends ElementalAbility {
	...
	public static boolean isEarthbendable(final Player player, final String abilityName, final Block block) {
		final Ninja nPlayer = Ninja.getBendingPlayer(player);
		if (nPlayer == null || !isEarthbendable(block.getType(), true, true, true) || PREVENT_EARTHBENDING.contains(block)
				|| RegionProtection.isRegionProtected(player, block.getLocation(), CoreAbility.getAbility(abilityName))) {
			return false;
		} else if (isMetal(block) && !nPlayer.canMetalbend()) {
			return false;
		} else if (isSand(block) && !nPlayer.canSandbend()) {
			return false;
		} else if (isLava(block) && !nPlayer.canLavabend()) {
			return false;
		}
		return true;
	}
	...
	public static boolean isMetalbendable(final Player player, final Material material) {
		final Ninja nPlayer = Ninja.getBendingPlayer(player);
		return nPlayer == null ? null : isMetal(material) && nPlayer.canMetalbend();
	}

	public static boolean isSandbendable(final Player player, final Material material) {
		final Ninja nPlayer = Ninja.getBendingPlayer(player);
		return nPlayer == null ? null : isSand(material) && nPlayer.canSandbend();
	}
