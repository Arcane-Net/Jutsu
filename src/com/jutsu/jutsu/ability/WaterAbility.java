...
public abstract class WaterAbility extends ElementalAbility {
	...
	public static boolean isIcebendable(final Player player, final Material material, final boolean onlyIce) {
		final Ninja nPlayer = Ninja.getBendingPlayer(player);
		return nPlayer == null ? null : isIce(material) && nPlayer.canIcebend() && (!onlyIce || material == Material.ICE);
	}

	public static boolean isPlantbendable(final Player player, final Material material, final boolean onlyLeaves) {
		final Ninja nPlayer = Ninja.getBendingPlayer(player);
		if (onlyLeaves) {
			return nPlayer == null ? null : isPlant(material) && nPlayer.canPlantbend() && isLeaves(material);
		} else {
			return nPlayer == null ? null : isPlant(material) && nPlayer.canPlantbend();
		}
	}
	...
	public static boolean isWaterbendable(final Player player, final String abilityName, final Block block) {
		final Ninja nPlayer = Ninja.getBendingPlayer(player);
		if (nPlayer == null || !isWaterbendable(block.getType())) {
			return false;
		}
		if (TempBlock.isTempBlock(block) && !isBendableWaterTempBlock(block)) {
			return false;
			...
			return true;
		} else if (isIce(block) && !nPlayer.canIcebend()) {
			return false;
		} else if (isPlant(block) && !nPlayer.canPlantbend()) {
			return false;
		}
		return true;
	}
