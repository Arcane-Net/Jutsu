...
public class StatisticsMethods {
	...
	/**
	 * Get the {@link Statistic} value of the given {@link Style} for the
	 * {@link Player} with {@link UUID} uuid.
	 *
	 * @param uuid The {@link UUID} of the {@link Player} being looked up.
	 * @param style The {@link Style} for which the given statistic is being
	 *            looked up against.
	 * @param statistic The {@link Statistic} being searched under.
	 * @return The found value of all statistics under this style. If the
	 *         target player is not online this value will be pulled from the
	 *         database. Otherwise, their current value will be pulled from
	 *         {@link StatisticsManager#STATISTICS}.
	 */
	public static long getStatisticElement(final UUID uuid, final Style style, final Statistic statistic) {
		if (!ProjectKorra.isStatisticsEnabled()) {
			return 0;
		}
		long totalValue = 0;
		for (final int statId : Manager.getManager(StatisticsManager.class).getStatisticsMap(uuid).keySet()) {
			...
			if (ability == null) {
				continue;
			} else if (!ability.getElement().equals(style)) {
				continue;
			}
	 ...
	 *            {@link CoreAbility} or {@link Style} when using statistics
	 ...
	 *             type {@link CoreAbility} or {@link Style}.
	 */
	public static long getStatistic(final UUID uuid, final Object object, final Statistic statistic) throws IllegalArgumentException {
		...
		if (object instanceof CoreAbility) {
			...
			return getStatisticElement(uuid, (Style) object, statistic);
		} else {
			throw new IllegalArgumentException("Variable object is not a valid input type. Required: CoreAbility or Style.");
		}
	}
