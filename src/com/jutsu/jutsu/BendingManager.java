...
public class BendingManager implements Runnable {
	...
	public void handleCooldowns() {
		for (Map.Entry<UUID, Ninja> entry : Ninja.getPlayers().entrySet()) {
			Ninja nPlayer = entry.getValue();

			nPlayer.removeOldCooldowns();
		}
	}
			...
			if (!this.times.containsKey(world)) {
				...
				if (this.times.get(world) && !ElementalAbility.isDay(world)) {
					...
					for (final Player player : world.getPlayers()) {
						...
						if (nPlayer.hasElement(Style.WATER) && player.hasPermission("jutsu.message.daymessage")) {
							player.sendMessage(Style.WATER.getColor() + getMoonriseMessage());
						}
						if (nPlayer.hasElement(Style.FIRE) && player.hasPermission("jutsu.message.nightmessage")) {
							player.sendMessage(Style.FIRE.getColor() + getSunsetMessage());
						}
					}
				}
				...
				if (!this.times.get(world) && ElementalAbility.isDay(world)) {
					...
					for (final Player player : world.getPlayers()) {
						...
						if (nPlayer.hasElement(Style.WATER) && player.hasPermission("jutsu.message.nightmessage")) {
							player.sendMessage(Style.WATER.getColor() + getMoonsetMessage());
						}
						if (nPlayer.hasElement(Style.FIRE) && player.hasPermission("jutsu.message.daymessage")) {
							player.sendMessage(Style.FIRE.getColor() + getSunriseMessage());
						}
					}
				}
			}
		...
		try (MCTiming timing = this.ACTIONBAR_STATUS_TIMING.startTiming()) {
			for (final Player player : Bukkit.getOnlinePlayers()) {
				if (Bloodbending.isBloodbent(player)) {
					ActionBar.sendActionBar(Style.BLOOD.getColor() + "* Bloodbent *", player);
				} else if (MetalClips.isControlled(player)) {
					ActionBar.sendActionBar(Style.METAL.getColor() + "* MetalClipped *", player);
				}
			}
		}
