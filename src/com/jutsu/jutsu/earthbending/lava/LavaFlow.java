...
public class LavaFlow extends LavaAbility {
	...
	public LavaFlow(final Player player, final AbilityType type) {
		...
		if (type == AbilityType.SHIFT) {
			...
			if (this.nPlayer.isOnCooldown("LavaFlow")) {
				this.removeSlowly();
				return;
			}
			this.start();
			...
			if (this.nPlayer.isOnCooldown("LavaFlow")) {
				...
				this.nPlayer.addCooldown("LavaFlow", cooldown);
			}
			this.start();
		}
	}
		...
		if (this.type == AbilityType.SHIFT) {
			...
			if (!this.player.isSneaking() && !this.removing) {
				if (this.affectedBlocks.size() > 0) {
					this.removeOnDelay();
					this.removing = true;
					this.nPlayer.addCooldown("LavaFlow", this.shiftCooldown);
				} else {
					this.removeSlowly();
				}
				return;
