...
public class Tremorsense extends EarthAbility {
	...
	private void tryToSetGlowBlock() {
		...
		if (!this.nPlayer.isTremorSensing()) {
			if (this.block != null) {
				this.remove();
			}
			return;
	...
	public void progress() {
		if (!this.nPlayer.canBendIgnoreBindsCooldowns(this) || this.player.getLocation().getBlock().getLightLevel() > this.lightThreshold) {
			this.remove();
			return;
		} else {
	...
	public static boolean canTremorSense(final Player player) {
		...
		if (nPlayer != null && nPlayer.canBendIgnoreBindsCooldowns(getAbility("Tremorsense"))) {
			return true;
		}
