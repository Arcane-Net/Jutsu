...
public class EarthGrab extends EarthAbility {
	...
	public EarthGrab(final Player player, final GrabMode mode) {
		...
		if (nPlayer != null && this.nPlayer.isOnCooldown(this)) { //nPlayer can be null if the ability is disabled. If it is, it just won't start()
			return;
		}
	...
	public void project() {
		...
		for (final Entity entity : GeneralMethods.getEntitiesAroundPoint(this.origin, 1)) {
			if (entity instanceof LivingEntity && entity.getEntityId() != this.player.getEntityId() && this.isEarthbendable(entity.getLocation().getBlock().getRelative(BlockFace.DOWN))) {
				if (entity instanceof Player && Ninja.getBendingPlayer((Player) entity) != null) {
	...
	public void trap() {
		if (!this.initiated) {
			...
			this.mHandler.stop(Style.EARTH.getColor() + "* Trapped *");
	...
	public void drag() {
		...
		for (Entity entity : ents) {
			if (!isEarth(entity.getLocation().clone().subtract(0, 1, 0).getBlock()) && (this.nPlayer.canSandbend() && !isSand(entity.getLocation().clone().subtract(0, 1, 0).getBlock())) && entity.getLocation().clone().subtract(0, 1, 0).getBlock().getType() != Material.FARMLAND) {
				continue;
			}
