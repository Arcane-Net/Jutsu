...
public class MetalClips extends MetalAbility {
	...
	public MetalClips(final Player player, final int abilityType) {
		...
		this.canLoot = player.hasPermission("jutsu.ability.MetalClips.loot");
		this.canUse4Clips = player.hasPermission("jutsu.ability.MetalClips.4clips");
		...
		this.canThrow = (getConfig().getBoolean("Abilities.Earth.MetalClips.ThrowEnabled") && player.hasPermission("jutsu.ability.metalclips.throw"));
		...
		if (abilityType == 0) {
			...
			if (this.nPlayer.isOnCooldown("MetalClips Magnet")) {
				return;
			}
	...
	public void shootMetal() {
		if (this.nPlayer.isOnCooldown("MetalClips Shoot")) {
			return;
		}
		this.nPlayer.addCooldown("MetalClips Shoot", this.shootCooldown);
	...
	public void crush() {
		if (this.nPlayer.isOnCooldown("MetalClips Crush")) {
			return;
		}
		this.nPlayer.addCooldown("MetalClips Crush", this.crushCooldown);
		DamageHandler.damageEntity(this.targetEntity, this.player, this.crushDamage, this);
	}

	@Override
	public void progress() {
		if (!this.nPlayer.canBendIgnoreCooldowns(this)) {
			this.remove();
			return;
		}
		...
		if (!this.player.isSneaking()) {
			if (this.isMagnetized) {
				this.nPlayer.addCooldown("MetalClips Magnet", this.magnetCooldown);
				this.remove();
				return;
			}
