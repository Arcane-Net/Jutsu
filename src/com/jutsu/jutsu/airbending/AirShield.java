...
public class AirShield extends AirAbility {
	...
	public AirShield(final Player player) {
		...
		if (this.nPlayer.isAvatarState() && hasAbility(player, AirShield.class) && this.isToggledByAvatarState) {
	...
	public void progress() {
		...
		if (this.player.getEyeLocation().getBlock().isLiquid()) {
			this.remove();
			return;
		} else if (!this.nPlayer.isAvatarState() || !this.isToggledByAvatarState) {
			if (!this.player.isSneaking() || !this.nPlayer.canBend(this)) {
				if (this.dynamicCooldown) {
					...
					this.nPlayer.addCooldown(this, reducedCooldown);
				} else {
		...
		} else if (!this.nPlayer.canBendIgnoreBinds(this)) {
			this.remove();
			return;
		}
		this.rotateShield();
