...
public class AcrobatStance extends ChiAbility {
	...
	public AcrobatStance(final Player player) {
		...
		final ChiAbility stance = this.nPlayer.getStance();
		if (stance != null) {
			stance.remove();
			if (stance instanceof AcrobatStance) {
				this.nPlayer.setStance(null);
				return;
			}
		}
		this.start();
		this.nPlayer.setStance(this);
		player.playSound(player.getLocation(), Sound.ENTITY_ENDER_DRAGON_HURT, 0.5F, 2F);
	}

	@Override
	public void progress() {
		if (!this.nPlayer.canBendIgnoreBinds(this) || !this.nPlayer.hasElement(Style.CHI)) {
			this.remove();
			return;
