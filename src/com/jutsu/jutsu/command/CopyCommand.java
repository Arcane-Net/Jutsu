...
public class CopyCommand extends PKCommand {
	...
	public CopyCommand() {
		super("copy", "/jutsu copy <Player> [Player]", ConfigManager.languageConfig.get().getString("Commands.Copy.Description"), new String[] { "copy", "co" });
	...
	private CompletableFuture<Boolean> assignAbilities(final CommandSender sender, final OfflinePlayer player, final Player player2, final boolean self) {
		...
		Ninja target = Ninja.getBendingPlayer(player2);
		CompletableFuture<Boolean> future = new CompletableFuture<>();
		Ninja.getOrLoadOfflineAsync(player).thenAccept(orig -> {
	...
	protected List<String> getTabCompletion(final CommandSender sender, final List<String> args) {
		if (!sender.hasPermission("jutsu.command.copy") || args.size() >= 2 || (args.size() >= 1 && !sender.hasPermission("jutsu.command.copy.assign"))) {
