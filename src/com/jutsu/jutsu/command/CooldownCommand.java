...
public class CooldownCommand extends PKCommand {
    ...
    public CooldownCommand() {
        super("cooldown", "/jutsu cooldown <view/set/reset> <player> [cooldown]", ConfigManager.getConfig().getString("Commands.Cooldown.Description"), new String[] {"cooldown", "cooldowns", "cd"});
    ...
    public void execute(CommandSender sender, List<String> list) {
        ...
        Ninja.getOrLoadOfflineAsync(oPlayer).thenAccept(nPlayer -> {
            if (Arrays.asList(new String[] {"view", "v"}).contains(list.get(0).toLowerCase())) {
                List<String> cooldowns = nPlayer.getCooldowns().entrySet().stream()
            ...
            if (list.size() > 3 && set) {
                ...
                if (cooldown.equals("*") || cooldown.equalsIgnoreCase("ALL")) {
                    if (time == 0) {
                        nPlayer.getCooldowns().keySet().forEach(nPlayer::removeCooldown); //We do this instead of clear() because we need to call the event
                ...
                String fixedCooldown = setCooldown(sender, nPlayer, cooldown, time);
                ...
                String fixedCooldown = setCooldown(sender, nPlayer, cooldown, 0);
                if (fixedCooldown == null) {
                    return;
                }
    ...
    private String setCooldown(CommandSender sender, OfflineBendingPlayer nPlayer, String cooldown, long time) {
        ...
        if (nPlayer.getPlayer().isOnline()) {
            final PlayerCooldownChangeEvent event = new PlayerCooldownChangeEvent(((Ninja)nPlayer).getPlayer(), cooldown, time, time <= 0 ? PlayerCooldownChangeEvent.Result.REMOVED : PlayerCooldownChangeEvent.Result.SET);
        ...
        if (!cancelled) {
            if (time <= 0) {
                nPlayer.getCooldowns().remove(fixedCooldown);
                return fixedCooldown;
            }

            Cooldown cooldownObject = nPlayer.getCooldowns().get(fixedCooldown);
            cooldownObject = new Cooldown(time + System.currentTimeMillis(), cooldownObject != null && cooldownObject.isDatabase());
            nPlayer.getCooldowns().put(fixedCooldown, cooldownObject);
            return fixedCooldown;
        }
