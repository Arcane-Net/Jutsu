...
/**
 * Executor for /jutsu toggle. Extends {@link PKCommand}.
 */
public class ToggleCommand extends PKCommand {
	...
	private Set<Style> cachedPassiveElements = new HashSet<>();

	public ToggleCommand() {
		super("toggle", "/jutsu toggle <All/Style> [Player]", ConfigManager.languageConfig.get().getString("Commands.Toggle.Description"), new String[] { "toggle", "t" });
	...
	public void execute(final CommandSender sender, final List<String> args) {
		if (!this.correctLength(sender, args.size(), 0, 2)) {
			return;
		} else if (args.size() == 0) { // jutsu toggle,
			...
			if (toggleableParam.equals("all") && this.hasPermission(sender, "all")) { // jutsu toggle all.
			...
			} else if (sender instanceof Player && args.size() == 1 && Style.fromString(toggleableParam) != null && !(Style.fromString(toggleableParam) instanceof SubElement)) {
				final Style e = Style.fromString(toggleableParam);
				if (!Ninja.getBendingPlayer(sender.getName()).hasElement(e)) {
				...
				nPlayer.toggleElement(e);

				if (nPlayer.isElementToggled(e)) {
					ChatUtil.sendBrandingMessage(sender, color + this.toggledOnSingleElement.replace("{style}", e.getName() + (e.getType() != null ? e.getType().getBending() : "")));
				} else {
					ChatUtil.sendBrandingMessage(sender, color + this.toggledOffSingleElement.replace("{style}", e.getName() + (e.getType() != null ? e.getType().getBending() : "")));
				}
			}  else if (sender instanceof Player && args.size() == 1 && !toggleableParam.equals("passives") && Style.fromString(toggleableParam.split("passives")[0]) != null) {
				final Style e = Style.fromString(toggleableParam.split("passives")[0]);
				if (!Ninja.getBendingPlayer(sender.getName()).hasElement(e)) {
					ChatUtil.sendBrandingMessage(sender, ChatColor.RED + this.wrongElement);
					return;
				}
				final ChatColor color = e.getColor();
				final Ninja nPlayer = Ninja.getBendingPlayer(sender.getName());
				nPlayer.togglePassive(e);

				if (nPlayer.isPassiveToggled(e)) {
					ChatUtil.sendBrandingMessage(sender, color + this.toggledOnSingleElementPassive.replace("{style}", e.getName() + (e.getType() != null ? e.getType().getBending() : "")));
				} else {
					ChatUtil.sendBrandingMessage(sender, color + this.toggledOffSingleElementPassive.replace("{style}", e.getName() + (e.getType() != null ? e.getType().getBending() : "")));
				}
			} else if (sender instanceof Player && args.size() == 1 && toggleableParam.equals("passives")) {
				Ninja nPlayer = Ninja.getBendingPlayer(sender.getName());

				if (nPlayer.isToggledPassives()) {
				...
				nPlayer.toggleAllPassives();
			} else {
				this.help(sender, false);
			}

		} else if (sender instanceof Player && args.size() == 2 && Style.fromString(args.get(0)) != null && !(Style.fromString(args.get(0)) instanceof SubElement)) {
			Style e = Style.fromString(args.get(0));
			...
			if (!Ninja.getBendingPlayer(target.getName()).hasElement(e)) {
			...
			final Ninja nPlayer = Ninja.getBendingPlayer(target.getName());
			final ChatColor color = e.getColor();

			if (nPlayer.isElementToggled(e)) {
				ChatUtil.sendBrandingMessage(sender, color + this.toggledOffOtherElementConfirm.replace("{target}", target.getName()).replace("{style}", e.getName() + (e.getType() != null ? e.getType().getBending() : "")));
				ChatUtil.sendBrandingMessage(target, color + this.toggledOffOtherElement.replace("{style}", e.getName() + (e.getType() != null ? e.getType().getBending() : "")).replace("{sender}", ChatColor.DARK_AQUA + sender.getName()));
			} else {
				ChatUtil.sendBrandingMessage(sender, color + this.toggledOnOtherElementConfirm.replace("{target}", target.getName()).replace("{style}", e.getName() + (e.getType() != null ? e.getType().getBending() : "")));
				ChatUtil.sendBrandingMessage(target, color + this.toggledOnOtherElement.replace("{style}", e.getName() + (e.getType() != null ? e.getType().getBending() : "")).replace("{sender}", ChatColor.DARK_AQUA + sender.getName()));
			}
			nPlayer.toggleElement(e);
		} else {
			this.help(sender, false);
	...
	public boolean hasAdminPermission(final CommandSender sender) {
		if (!sender.hasPermission("jutsu.admin.toggle")) {
	...
	protected List<String> getTabCompletion(final CommandSender sender, final List<String> args) {
		if (args.size() >= 2 || !sender.hasPermission("jutsu.command.toggle.others")) {
		...
		if (args.size() == 0) {
			...
			for (final Style e : Style.getAllElements()) {
				styles.add(e.getName());
			}
			cachedPassiveElements.forEach(e -> styles.add(e.getName() + "Passives"));
