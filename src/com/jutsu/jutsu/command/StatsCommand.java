...
public class StatsCommand extends PKCommand {
	...
	public StatsCommand() {
		super("stats", "/jutsu stats <get/leaderboard> <ability/style/all> <statistic> [player/page]", ConfigManager.languageConfig.get().getString("Commands.Stats.Description"), new String[] { "statistics", "stats" });
	...
	public void execute(final CommandSender sender, final List<String> args) {
		...
		final Style style = Style.getElement(args.get(1));
		Object object = null;
		if (ability != null) {
			object = ability;
		} else if (style != null) {
	...
	public String getTarget(final Object object, final Statistic statistic, final Player target) {
		...
		if (object instanceof CoreAbility) {
			...
			message = message.replace("%object%", style.getName()).replace("%player%", target.getName()).replace("%value%", String.valueOf(value));
		} else {
	...
	public List<String> getLeaderboard(final CommandSender sender, final Object object, final Statistic statistic, final int page) {
		...
		if (object instanceof CoreAbility) {
			...
			final Style style = (Style) object;
			title = title.replace("%object%", style.getName());
		} else {
