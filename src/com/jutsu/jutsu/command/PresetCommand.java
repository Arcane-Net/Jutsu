...
/**
 * Executor for /jutsu preset. Extends {@link PKCommand}.
 */
public class PresetCommand extends PKCommand {
	...
	public PresetCommand() {
		super("preset", "/jutsu preset <Bind/Create/Delete/List> [Preset]", ConfigManager.languageConfig.get().getString("Commands.Preset.Description"), new String[] { "preset", "presets", "pre", "set", "p" });
	...
	public void execute(final CommandSender sender, final List<String> args) {
		...
		if (Arrays.asList(listaliases).contains(args.get(0)) && this.hasPermission(sender, "list")) {
		...
		} else if (Arrays.asList(deletealiases).contains(args.get(0)) && this.hasPermission(sender, "delete")) { // jutsu preset delete name.
		...
		} else if (Arrays.asList(bindaliases).contains(args.get(0)) && this.hasPermission(sender, "bind")) { // jutsu preset bind name.
			if (args.size() < 3) {
				...
				} else if (nPlayer.isPermaRemoved()) {
				...
				if (player2 != null && player2.isOnline()) {
					Ninja bPlayer2 = Ninja.getBendingPlayer(player2);
		...
		} else if (Arrays.asList(createaliases).contains(args.get(0)) && this.hasPermission(sender, "create")) { // jutsu preset create name.
			...
			final HashMap<Integer, String> abilities = (HashMap<Integer, String>) nPlayer.getAbilities().clone();
	...
	protected List<String> getTabCompletion(final CommandSender sender, final List<String> args) {
		if (args.size() >= 3 || !sender.hasPermission("jutsu.command.preset") || !(sender instanceof Player)) {
		...
		if (args.size() == 0) {
			...
			if (sender.hasPermission("jutsu.command.preset.bind.external")
