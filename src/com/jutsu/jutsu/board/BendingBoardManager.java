...
public final class BendingBoardManager {
	...
	/**
	 * Toggles the bendingboard for the given player if the board is enabled and they are in a jutsu enabled world
	...
	public static Optional<BendingBoard> getBoard(Player player) {
		...
		if (!scoreboardPlayers.containsKey(player)) {
			...
			scoreboardPlayers.put(player, new BendingBoard(nPlayer));
		}
	 ...
	 * Adds the internal cooldown name and color to the map of tracked abilities so as they can appear on the jutsu board.
	...
	/**
	 * Load into memory the list of players who have toggled the jutsu board off.
	 */
	 ...
	 * Removes the board instance and stores the player's toggle preference for the jutsu board in the database
	 * @param player
	 */
