...
public class WaterSpout extends WaterAbility {
	...
	public void progress() {
		...
		if (this.player.isDead() || !this.player.isOnline() || !this.nPlayer.canBendIgnoreBindsCooldowns(this)) {
			this.remove();
			return;
	...
	private double spoutableWaterHeight(final Location location) {
		...
		for (int i = 0; i < this.maxHeight; i++) {
			...
			if (TempBlock.get(blocki) == null || !this.blocks.contains(TempBlock.get(blocki))) {
				...
				if ((!ElementalAbility.isAir(blocki.getType()) && (!isPlant(blocki) || !this.nPlayer.canPlantbend()))) {
					this.revertBaseBlock();
					return -1;
				}
			}
