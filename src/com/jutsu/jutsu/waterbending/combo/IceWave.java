...
public class IceWave extends IceAbility implements ComboAbility {
	...
	public IceWave(final Player player) {
		...
		if (this.nPlayer.isOnCooldown("IceWave") && !this.nPlayer.isAvatarState()) {
			this.remove();
			return;
		}
	...
	public void progress() {
		...
		if (this.origin == null && WaterSpoutWave.containsType(this.player, WaterSpoutWave.AbilityType.RELEASE)) {
			this.nPlayer.addCooldown("IceWave", this.cooldown);
	...
	public void remove() {
		super.remove();
		this.nPlayer.addCooldown("WaterWave", this.cooldown);
	}
