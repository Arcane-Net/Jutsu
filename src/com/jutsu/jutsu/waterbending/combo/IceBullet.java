...
public class IceBullet extends IceAbility implements ComboAbility {
	...
	public void progress() {
		...
		if (this.origin == null) {
			if (this.nPlayer.isOnCooldown("IceBullet")) {
				this.remove();
				return;
			}

			final Block waterBlock = BlockSource.getWaterSourceBlock(this.player, this.range, ClickType.SHIFT_DOWN, true, true, this.nPlayer.canPlantbend());
			if (waterBlock == null) {
				this.remove();
				return;
			}
				...
				if (timeDiff < this.shootTime) {
					if (this.rightClicks > 0 || this.leftClicks > 0) {
						if (this.leftClicks > 0) {
							this.leftClicks = 0;
							this.nPlayer.addCooldown("IceBullet Left", this.shotcooldown);
						} else if (this.rightClicks > 0) {
							this.rightClicks = 0;
							this.nPlayer.addCooldown("IceBullet Right", this.shotcooldown);
						}
	...
	public void doLeftClick() {
		if (this.nPlayer.isOnCooldown("IceBullet Left")) {
			return;
		}
		this.leftClicks++;
	}
	...
	public void doRightClick() {
		if (this.nPlayer.isOnCooldown("IceBullet Right")) {
			return;
		}
		this.rightClicks++;
	}
