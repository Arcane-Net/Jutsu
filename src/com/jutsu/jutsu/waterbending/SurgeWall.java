...
public class SurgeWall extends WaterAbility {
	...
	public SurgeWall(final Player player) {
		...
		if (wave != null && !wave.isProgressing() && !this.nPlayer.isOnCooldown("SurgeWave")) {
			wave.moveWater();
			return;
		}
		...
		} else if (!this.nPlayer.isOnCooldown("SurgeWall") && this.prepare()) {
			this.start();
			this.time = System.currentTimeMillis();
			return;
		}

		if (this.nPlayer.isOnCooldown("SurgeWave") || player.isSneaking()) {
			return;
	...
	public boolean prepare() {
		this.cancelPrevious();
		final Block block = BlockSource.getWaterSourceBlock(this.player, this.range, ClickType.LEFT_CLICK, true, true, this.nPlayer.canPlantbend());
	...
	public void moveWater() {
		if (this.sourceBlock != null) {
			...
			if (this.targetDestination.distanceSquared(this.location) <= 1) {
				...
				this.nPlayer.addCooldown("SurgeWall", this.cooldown);
	...
	public void progress() {
		...
		if (System.currentTimeMillis() - this.time >= this.interval) {
			this.time = System.currentTimeMillis();
			final boolean matchesName = this.nPlayer.getBoundAbilityName().equals(this.getName());
	...
	public static void form(final Player player) {
		...
		if (wall == null) {
			final Block source = BlockSource.getWaterSourceBlock(player, range, ClickType.SHIFT_DOWN, true, true, nPlayer.canPlantbend());

			if (wave == null && source == null && WaterReturn.hasWaterBottle(player)) {
				if (nPlayer.isOnCooldown("SurgeWall")) {
					return;
				}
			...
			if (!nPlayer.isOnCooldown("SurgeWave")) {
				wave = new SurgeWave(player);
			}
			return;
		} else {
