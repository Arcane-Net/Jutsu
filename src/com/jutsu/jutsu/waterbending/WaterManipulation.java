...
public class WaterManipulation extends WaterAbility {
	...
	private static Block prepare(final Player player, final double selectRange) {
		...
		final Block block = BlockSource.getWaterSourceBlock(player, selectRange, ClickType.SHIFT_DOWN, true, true, nPlayer.canPlantbend());
	...
	public void progress() {
		...
		if (System.currentTimeMillis() - this.time >= this.interval) {
			if (!this.progressing && !this.falling && !this.nPlayer.getBoundAbilityName().equalsIgnoreCase(this.getName())) {
				this.remove();
				return;
			}
			...
			if (this.falling) {
				...
				if (!this.progressing) {
					if (!(isWater(this.sourceBlock.getType()) || (isIce(this.sourceBlock) && this.nPlayer.canIcebend()) || (isSnow(this.sourceBlock) && this.nPlayer.canIcebend()) || (isPlant(this.sourceBlock) && this.nPlayer.canPlantbend()))) {
						this.remove();
						return;
					}
	...
	public static void moveWater(final Player player) {
		...
		if (nPlayer.isOnCooldown("WaterManipulation")) {
