...
public class OctopusForm extends WaterAbility {
	...
	public OctopusForm(final Player player) {
		...
		if (!player.isSneaking()) {
			this.sourceBlock = BlockSource.getWaterSourceBlock(player, this.range, ClickType.LEFT_CLICK, true, true, this.nPlayer.canPlantbend());
		}
	...
	private void attack() {
		if (!this.formed || this.nPlayer.isOnCooldown("OctopusAttack")) {
			return;
		}
		this.nPlayer.addCooldown("OctopusAttack", this.usageCooldown);
	...
	private void affect(final Location location) {
		for (final Entity entity : GeneralMethods.getEntitiesAroundPoint(location, this.attackRange)) {
			...
			final double knock = this.nPlayer.isAvatarState() ? AvatarState.getValue(this.knockback) : this.knockback;
