...
public class PlantTether extends PlantAbility {
	...
	public PlantTether(Player player) {
		...
		if (this.nPlayer.isOnCooldown("PlantTether")) {
			return;
		}

		this.setFields();
		this.currentState = State.INITIAL;
		this.source = BlockSource.getWaterSourceBlock(this.player, this.selectRange, ClickType.SHIFT_DOWN, false, false, this.nPlayer.canPlantbend(), false, false);
		if (this.source != null) {
	...
	public void progress() {
		switch (this.currentState) {
			case SEARCHING:
				Block newSource = BlockSource.getWaterSourceBlock(this.player, this.selectRange, ClickType.SHIFT_DOWN, false, false, this.nPlayer.canPlantbend(), false, false);
			...
			case ENTITYPULL:
				...
				if ((this.timeOut > 15000) || (this.target.getLocation().distance(this.originLoc) < 0.5)) {
					this.nPlayer.addCooldown("PlantTether", this.getCooldown());
					this.remove();
				}
				break;
			case INITIAL:
