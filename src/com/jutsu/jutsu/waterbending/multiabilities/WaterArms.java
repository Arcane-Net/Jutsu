...
public class WaterArms extends WaterAbility {
	...
	public WaterArms(final Player player) {
		...
		if (oldArms != null) {
			if (player.isSneaking()) {
				...
				switch (player.getInventory().getHeldItemSlot()) {
					case 0:
						if (player.hasPermission("jutsu.ability.WaterArms.Pull")) {
							new WaterArmsWhip(player, Whip.PULL);
						}
						break;
					case 1:
						if (player.hasPermission("jutsu.ability.WaterArms.Punch")) {
							new WaterArmsWhip(player, Whip.PUNCH);
						}
						break;
					case 2:
						if (player.hasPermission("jutsu.ability.WaterArms.Grapple")) {
							new WaterArmsWhip(player, Whip.GRAPPLE);
						}
						break;
					case 3:
						if (player.hasPermission("jutsu.ability.WaterArms.Grab")) {
							new WaterArmsWhip(player, Whip.GRAB);
						}
						break;
					case 4:
						if (player.hasPermission("jutsu.ability.WaterArms.Freeze") && this.nPlayer.canIcebend()) {
							new WaterArmsFreeze(player);
						}
						break;
					case 5:
						if (player.hasPermission("jutsu.ability.WaterArms.Spear")) {
							if (this.nPlayer.canIcebend()) {
		...
		if (this.nPlayer.canBend(this) && this.prepare()) {
			this.start();
			MultiAbilityManager.bindMultiAbility(player, "WaterArms");

			if (ChatColor.stripColor(this.nPlayer.getBoundAbilityName()) == null) {
				this.remove();
				return;
			}
		}
	...
	public void progress() {
		if (!this.world.equals(this.player.getWorld()) || !this.nPlayer.canBendIgnoreBindsCooldowns(this) || !this.nPlayer.hasElement(Style.WATER)) {
			this.remove();
			return;
		} else if (!this.nPlayer.isToggled()) {
			this.remove();
			return;
	...
	public boolean displayRightArm() {
		...
		for (int j = 1; j <= this.initLength; j++) {
			...
			if (j >= 1 && this.selectedSlot == this.freezeSlot && this.nPlayer.canIcebend()) {
	...
	public void remove() {
		...
		if (this.player.isOnline()) {
			this.nPlayer.addCooldown("WaterArms", this.cooldown);
		}
	...
	public void displayBoundMsg(final int slot) {
		final String name = this.nPlayer.getAbilities().get(slot);
		if (name != null) {
