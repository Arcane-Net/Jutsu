...
public class PhaseChange extends IceAbility {
	...
	public void progress() {
		...
		if (this.active_types.contains(PhaseChangeType.MELT)) {
			if (!this.player.isSneaking() || !this.nPlayer.canBend(this)) {
				...
				this.nPlayer.addCooldown("PhaseChangeMelt", this.meltCooldown);
				this.meltRadius = 1;
				this.meltTicks = 0;
				return;
			}
	...
	public void startNewType(final PhaseChangeType type) {
		if (type == PhaseChangeType.MELT) {
			if (this.nPlayer.isOnCooldown("PhaseChangeMelt")) {
				return;
			}
		}
	...
	public void freezeArea(final Location center, final double radius, final PhaseChangeType type) {
		if (type == PhaseChangeType.FREEZE) {
			if (this.nPlayer.isOnCooldown("PhaseChangeFreeze")) {
				return;
			}
		}
		...
		if (!this.blocks.isEmpty()) {
			if (type == PhaseChangeType.FREEZE) {
				this.nPlayer.addCooldown("PhaseChangeFreeze", this.freezeCooldown);
			}
		}
	}
