...
public class IceSpikeBlast extends IceAbility {
	...
	public IceSpikeBlast(final Player player) {
		...
		if (this.nPlayer.isOnCooldown("IceSpikeBlast")) {
			return;
		}
		...
		if (!this.nPlayer.canBend(this) || !this.nPlayer.canIcebend()) {
			return;
		}
		...
		block(player);
		this.sourceBlock = getWaterSourceBlock(player, this.range, this.nPlayer.canPlantbend());
	...
	public void progress() {
		if (!this.nPlayer.canBendIgnoreBindsCooldowns(this)) {
			...
			return;
		} else if (!this.nPlayer.getBoundAbilityName().equals(this.getName()) && this.prepared) {
			this.remove();
			return;
		}
	...
	public static void activate(final Player player) {
		...
		if (nPlayer.isOnCooldown("IceSpikeBlast")) {
			return;
		}
		...
		for (final IceSpikeBlast ice : getAbilities(player, IceSpikeBlast.class)) {
			if (ice.prepared) {
				ice.throwIce();
				nPlayer.addCooldown("IceSpikeBlast", ice.getCooldown());
				activate = true;
			}
		}
