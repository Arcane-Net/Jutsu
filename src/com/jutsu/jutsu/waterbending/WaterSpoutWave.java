...
public class WaterSpoutWave extends WaterAbility {
	...
	public void progress() {
		...
		if (this.type != AbilityType.RELEASE) {
			if (!this.player.hasPermission("jutsu.ability.WaterSpout.Wave")) {
				this.remove();
				return;
			} else if (!this.nPlayer.getBoundAbilityName().equals(this.getName())) {
				this.remove();
				return;
			}
		}
		...
		if (this.type == AbilityType.CLICK) {
			...
			if (this.animation == AnimateState.SHRINK) {
				...
				if ((System.currentTimeMillis() - this.time > this.flightDuration && !this.nPlayer.isAvatarState()) || this.player.isSneaking()) {
					this.remove();
					return;
				}
	...
	public Style getElement() {
		return this.isIceWave() ? Style.ICE : Style.WATER;
	}
