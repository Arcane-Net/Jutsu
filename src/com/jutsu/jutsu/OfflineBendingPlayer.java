...
public class OfflineBendingPlayer {
    ...
    /**
     * ConcurrentHashMap that contains all instances of ALL Ninja, with UUID
     * key.
     */
    protected static final Map<UUID, OfflineBendingPlayer> PLAYERS = new ConcurrentHashMap<>();

    /**
     * ConcurrentHashMap that contains all instances of online Ninja, with UUID
     * key.
     */
    protected static final Map<UUID, Ninja> ONLINE_PLAYERS = new ConcurrentHashMap<>();
    ...
    protected final Set<Style> toggledElements = new HashSet<>();
    protected final Set<Style> toggledPassives = new HashSet<>();
    ...
    protected static CompletableFuture<OfflineBendingPlayer> loadAsync(@NotNull final UUID uuid, boolean onStartup) {
        ...
        if (PLAYERS.get(uuid) != null) {
            ...
            if (offlinePlayer.isOnline() && !(oBendingPlayer instanceof Ninja)) {
                oBendingPlayer = convertToOnline(oBendingPlayer); //Convert to online instance
                ((Ninja)oBendingPlayer).postLoad();
            }
            if (!(oBendingPlayer instanceof Ninja)) {
        ...
        Runnable runnable = () -> {
            OfflineBendingPlayer nPlayer = new OfflineBendingPlayer(offlinePlayer);
            if (offlinePlayer.isOnline()) {
                nPlayer = new Ninja(((Player)offlinePlayer));
                ONLINE_PLAYERS.put(uuid, (Ninja)nPlayer);
            }

            PLAYERS.put(uuid, nPlayer);
            ...
            try {
                if (!rs2.next()) { // Data doesn't exist, we want a completely new player.
                    ...
                    Bukkit.getScheduler().runTask(ProjectKorra.plugin, () -> ProjectKorra.log.info("Created new Ninja for " + offlinePlayer.getName()));
                    OfflineBendingPlayer newPlayer;
                    if (offlinePlayer.isOnline()) {
                        newPlayer = new Ninja((Player)offlinePlayer);
                        //Call postLoad() on the main thread and wait for it to complete
                        Bukkit.getScheduler().callSyncMethod(ProjectKorra.plugin, () -> {
                            ((Ninja)newPlayer).postLoad();
                            return true;
                        }).get();
                        ONLINE_PLAYERS.put(uuid, (Ninja) newPlayer);
                    } else {
                    ...
                    final String elementField = rs2.getString("style");
                    ...
                    if (elementField != null && !elementField.equalsIgnoreCase("NULL")) {
                        ...
                        if (split.length > 0 && !split[0].equals("")) { // Player has an style.
                            if (split[0].contains("a")) {
                                nPlayer.styles.add(Style.AIR);
                            }
                            if (split[0].contains("w")) {
                                nPlayer.styles.add(Style.WATER);
                            }
                            if (split[0].contains("e")) {
                                nPlayer.styles.add(Style.EARTH);
                            }
                            if (split[0].contains("f")) {
                                nPlayer.styles.add(Style.FIRE);
                            }
                            if (split[0].contains("c")) {
                                nPlayer.styles.add(Style.CHI);
                            }
                        }
                        if (hasAddon) {
                            /*
                             ...
                             * styles would = null. To work around this, we
                             * keep trying to load in the styles from the
                            ...
                            OfflineBendingPlayer finalBPlayer = nPlayer;
                            Predicate<List<String>> func = (styles) -> {
                                if (System.currentTimeMillis() - startTime > timeoutLength) {
                                    ProjectKorra.log.severe("ProjectKorra has timed out after attempting to load in the following addon styles: " + addonClone.toString());
                                    ProjectKorra.log.severe("These styles have taken too long to load in, resulting in users having lost these style.");
                                    return true;
                                } else {
                                    ProjectKorra.log.info("Attempting to load in the following addon styles... " + styles.toString());
                                    for (final String addon : styles) {
                                        if (Style.getElement(addon) != null) {
                                            finalBPlayer.styles.add(Style.getElement(addon));
                                            styles.remove(addon);
                                        }
                                    }
                                    if (styles.isEmpty()) {
                                        ProjectKorra.log.info("Successfully loaded in all addon styles!");
                                        return true;
                                    }
                                }
                                return false;
                            ...
                            if (onStartup) { //If we are doing this on startup, addon styles aren't loaded yet. So do this async
                                new BukkitRunnable() {
                                    @Override
                                    public void run() {
                            ...
                            } else func.test(addonClone); //Addon styles should be loaded so
                        }
                    }
                        ...
                        if (subelementField.equals("-")) {
                            ...
                            if (offlinePlayer instanceof Player) { //Only if the player is online though
                                ...
                                for (final SubElement sub : Style.getAllSubElements()) {
                                    if (sub instanceof Style.MultiSubElement) { //If it's a multisub, check if they have any of the parent style and perm for the sub of that parent
                                        for (Style parent : ((Style.MultiSubElement) sub).getParentElements()) {
                                            if (((Player) offlinePlayer).hasPermission("jutsu." + parent.getName() + "." + sub.getName() + sub.getType().getBending()) && nPlayer.styles.contains(sub.getParentElement())) {
                                                nPlayer.subelements.add(sub);
                                                continue subloop;
                                            }
                                        }
                                    } else if (((Player)offlinePlayer).hasPermission("jutsu." + sub.getParentElement().getName().toLowerCase() + "." + sub.getName().toLowerCase() + sub.getType().getBending())
                                            && nPlayer.styles.contains(sub.getParentElement())) {
                                        nPlayer.subelements.add(sub);
                                        shouldSave = true;
                                    }
                                }
                                if (shouldSave) nPlayer.saveSubElements();
                            }
                        } else if (split.length > 0 && !split[0].equals("")) {
                            if (split[0].contains("m")) {
                                nPlayer.subelements.add(Style.METAL);
                            }
                            if (split[0].contains("v")) {
                                nPlayer.subelements.add(Style.LAVA);
                            }
                            if (split[0].contains("s")) {
                                nPlayer.subelements.add(Style.SAND);
                            }
                            if (split[0].contains("c")) {
                                nPlayer.subelements.add(Style.COMBUSTION);
                            }
                            if (split[0].contains("l")) {
                                nPlayer.subelements.add(Style.LIGHTNING);
                            }
                            if (split[0].contains("t")) {
                                nPlayer.subelements.add(Style.SPIRITUAL);
                            }
                            if (split[0].contains("f")) {
                                nPlayer.subelements.add(Style.FLIGHT);
                            }
                            if (split[0].contains("i")) {
                                nPlayer.subelements.add(Style.ICE);
                            }
                            if (split[0].contains("h")) {
                                nPlayer.subelements.add(Style.HEALING);
                            }
                            if (split[0].contains("b")) {
                                nPlayer.subelements.add(Style.BLOOD);
                            }
                            if (split[0].contains("p")) {
                                nPlayer.subelements.add(Style.PLANT);
                            }
                            if (split[0].contains("r")) {
                                nPlayer.subelements.add(Style.BLUE_FIRE);
                            }
                        }
                        if (hasAddon) {
                            ...
                            final long startTime = System.currentTimeMillis();
                            final long timeoutLength = 5_000; // How long until it should time out attempting to load addons in.
                            OfflineBendingPlayer finalBPlayer1 = nPlayer;
                            Predicate<List<String>> func = (styles) -> {
                                if (System.currentTimeMillis() - startTime > timeoutLength) {
                                    ...
                                    ProjectKorra.log.info("Attempting to load in the following addon subelements... " + styles.toString());
                                    for (final String addon : styles) {
                                        if (Style.getElement(addon) != null && Style.getElement(addon) instanceof SubElement) {
                                            finalBPlayer1.subelements.add((SubElement) Style.getElement(addon));
                                            styles.remove(addon);
                                        }
                                    }
                            ...
                            } else func.test(addonClone); //Addon styles should be loaded by now
                        }
                    }
                    ...
                    final long startTime = System.currentTimeMillis();
                    final long timeoutLength = 5_000; // How long until it should time out attempting to load addons in.
                    OfflineBendingPlayer finalBPlayer2 = nPlayer;
                    ...
                    } else func.test(abilitiesClone); //Addon styles should be loaded by now

                    //Load permaRemove
                    if (permaremovedField != null && permaremovedField.equalsIgnoreCase("true")) nPlayer.permaRemoved = true;
                    ...
                    if (ProjectKorra.isDatabaseCooldownsEnabled()) {
                        try (ResultSet rs = DBConnection.sql.readQuery("SELECT * FROM pk_cooldowns WHERE uuid = '" + uuid.toString() + "'")) {
                            while (rs.next()) {
                                ...
                                final String name = nPlayer.cooldownManager.getCooldownName(cooldownId);
                                nPlayer.cooldowns.put(name, new Cooldown(value, true));
                            }
                    ...
                    nPlayer.loading = false;
                    //Call postLoad() on the main thread and wait for it to complete
                    if (nPlayer instanceof Ninja) {
                        Ninja finalBPlayer3 = (Ninja) nPlayer;
                        Bukkit.getScheduler().callSyncMethod(ProjectKorra.plugin, () -> {
    ...
    /**
     * Saves the subelements of a Ninja to the database.
     */
    public void saveSubElements() {
        final StringBuilder subs = new StringBuilder();
        if (this.hasSubElement(Style.METAL)) {
            subs.append("m");
        }
        if (this.hasSubElement(Style.LAVA)) {
            subs.append("v");
        }
        if (this.hasSubElement(Style.SAND)) {
            subs.append("s");
        }
        if (this.hasSubElement(Style.COMBUSTION)) {
            subs.append("c");
        }
        if (this.hasSubElement(Style.LIGHTNING)) {
            subs.append("l");
        }
        if (this.hasSubElement(Style.SPIRITUAL)) {
            subs.append("t");
        }
        if (this.hasSubElement(Style.FLIGHT)) {
            subs.append("f");
        }
        if (this.hasSubElement(Style.ICE)) {
            subs.append("i");
        }
        if (this.hasSubElement(Style.HEALING)) {
            subs.append("h");
        }
        if (this.hasSubElement(Style.BLOOD)) {
            subs.append("b");
        }
        if (this.hasSubElement(Style.PLANT)) {
            subs.append("p");
        }
        if (this.hasSubElement(Style.BLUE_FIRE)) {
            subs.append("r");
        }
        boolean hasAddon = false;
        List<SubElement> addonSubs = Arrays.asList(Style.getAddonSubElements());
        for (final Style style : this.getSubElements()) {
            if (addonSubs.contains(style)) {
                if (!hasAddon) {
                    hasAddon = true;
                    subs.append(";");
                }
                subs.append(style.getName() + ",");
            }
        }
    ...
    /**
     * Saves the styles of a Ninja to the database.
     */
    public void saveElements() {
        final StringBuilder styles = new StringBuilder();
        if (this.hasElement(Style.AIR)) {
            styles.append("a");
        }
        if (this.hasElement(Style.WATER)) {
            styles.append("w");
        }
        if (this.hasElement(Style.EARTH)) {
            styles.append("e");
        }
        if (this.hasElement(Style.FIRE)) {
            styles.append("f");
        }
        if (this.hasElement(Style.CHI)) {
            styles.append("c");
        }
        boolean hasAddon = false;
        List<Style> addonElements = Arrays.asList(Style.getAddonElements());
        for (final Style style : this.getElements()) {
            if (addonElements.contains(style)) {
                if (!hasAddon) {
                    hasAddon = true;
                    styles.append(";");
                }
                styles.append(style.getName() + ",");
            }
        }
        ...
        DBConnection.sql.modifyQuery("UPDATE pk_players SET style = '" + styles.toString() + "' WHERE uuid = '" + uuid + "'");
    }
    ...
    public void bindAbility(final String ability, final int slot) {
        boolean realPlayer = this instanceof Ninja;
    ...
    public void saveAbility(final String ability, final int slot) {
        ...
        if (this instanceof Ninja && MultiAbilityManager.playerAbilities.containsKey((Player)this.getPlayer())) {
            return;
        }
    ...
    /**
     * Gets the list of styles the {@link Ninja} knows.
     *
    ...
    /**
     * Gets the list of subelements the {@link Ninja} knows.
     *
    ...
    /**
     * Gets the unique identifier of the {@link Ninja}.
     *
     * @return the uuid
     */
    public UUID getUUID() {
    ...
    /**
     * Gets the name of the {@link Ninja}.
     *
    ...
    /**
     * Gets the map of abilities that the {@link Ninja} knows.
     *
    ...
    /**
     * Adds an style to the {@link Ninja}'s known list.
     *
    ...
    /**
     * Adds a subelement to the {@link Ninja}'s known list.
     *
    ...
    /**
     * Checks to see if a player can bend a specific sub style. Used when
     * checking addon sub styles.
     *
    ...
    /**
     * Gets the map of cooldowns of the {@link Ninja}.
     *
    ...
    /**
     * Applies a cooldown for an ability to the current Ninja.
    ...
    /**
     * @return Returns true if this Ninja is fully loaded
     */
    ...
    /**
     * Checks to see if the {@link Ninja} knows a specific style.
     *
     * @param style The style to check
     * @return true If the player knows the style
     */
    public boolean hasElement(@NotNull final Style style) {
        if (style == Style.AVATAR) {
            // At the moment we'll allow for both permissions to return true.
            // Later on we can consider deleting the jutsu.ability.avatarstate option.
            return this.player instanceof Player && ((Player)this.player).hasPermission("jutsu.avatar");
        } else if (!(style instanceof SubElement)) {
            return this.styles.contains(style);
        } else {
            return this.hasSubElement((SubElement) style);
        }
    }

    /**
     * Checks to see if the {@link Ninja} has a specific subelement.
     *
    ...
    /**
     * Checks to see if the {@link Ninja} is a bender.
     *
    ...
    /**
     * Checks if the {@link Ninja} has the specified style toggled on
     *
     * @param style The style to check
     * @return true if the style is toggled on
     */
    public boolean isElementToggled(final Style style) {
        return !this.toggledElements.contains(style);
    }

    /**
     * Checks if the {@link Ninja} has the specified style's passives toggled on
     *
     * @param style The style to check
     * @return true if the style's passives are toggled on
     */
    public boolean isPassiveToggled(final Style style) {
        return !this.toggledPassives.contains(style);
    }

    /**
     * Checks if the {@link Ninja} is permaremoved.
     *
    ...
    /**
     * Checks if the {@link Ninja} has jutsu toggled on.
     *
     * @return true If jutsu is toggled on
     */
    ...
    /**
     * Checks if the {@link Ninja} has jutsu passives toggled on.
     *
    ...
    /**
     * Sets the {@link Ninja}'s abilities. This method also saves the
    ...
    /**
     * Sets the {@link Ninja}'s style. If the player had styles
     * before they will be overwritten.
     *
     * @param style The style to set
     */
    public void setElement(@NotNull final Style style) {
        this.styles.clear();
        this.styles.add(style);
    }

    /**
     * Sets the permanent removed state of the {@link Ninja}.
     *
    ...
    public void toggleElement(@NotNull final Style style) {
        if (this.toggledElements.contains(style)) {
            this.toggledElements.remove(style);
        } else {
            this.toggledElements.add(style);
        }
    }

    public void togglePassive(@NotNull final Style style) {
        if (this.toggledPassives.contains(style)) {
            this.toggledPassives.remove(style);
        } else {
            this.toggledPassives.add(style);
        }
    }
     ...
     *
     * @return true If player has permission node "jutsu.earth.bloodbending"
     */
    public boolean canBloodbend() {
        return this.subelements.contains(Style.BLOOD);
    }
    ...
    public boolean canCombustionbend() {
        return this.subelements.contains(Style.COMBUSTION);
    }

    public boolean canIcebend() {
        return this.subelements.contains(Style.ICE);
    }

    /**
     * Checks to see if a player can LavaBend.
     *
     * @return true If player has permission node "jutsu.earth.lavabending"
     */
    public boolean canLavabend() {
        return this.subelements.contains(Style.LAVA);
    }

    public boolean canLightningbend() {
        return this.subelements.contains(Style.LIGHTNING);
    }

    /**
     * Checks to see if a player can MetalBend.
     *
     * @return true If player has permission node "jutsu.earth.metalbending"
     */
    public boolean canMetalbend() {
        return this.subelements.contains(Style.METAL);
    }

    /**
     * Checks to see if a player can PlantBend.
     *
     * @return true If player has permission node "jutsu.ability.plantbending"
     */
    public boolean canPlantbend() {
        return this.subelements.contains(Style.PLANT);
    }

    /**
     * Checks to see if a player can SandBend.
     *
     * @return true If player has permission node "jutsu.earth.sandbending"
     */
    public boolean canSandbend() {
        return this.subelements.contains(Style.SAND);
    }

    /**
     * Checks to see if a player can use Flight.
     *
     * @return true If player has permission node "jutsu.air.flight"
     */
    public boolean canUseFlight() {
        return this.subelements.contains(Style.FLIGHT);
    }
     ...
     *         "jutsu.air.spiritualprojection"
     */
    public boolean canUseSpiritualProjection() {
        return this.subelements.contains(Style.SPIRITUAL);
    }

    /**
     * Checks to see if a player can use Water Healing.
     *
     * @return true If player has permission node "jutsu.water.healing"
     */
    public boolean canWaterHeal() {
        return this.subelements.contains(Style.HEALING);
    }
    ...
    protected static Ninja convertToOnline(@NotNull OfflineBendingPlayer offlineBendingPlayer) {
        Player player = Bukkit.getPlayer(offlineBendingPlayer.getUUID());
        if (player == null) {
            return null;
        }
        Ninja bendingPlayer = new Ninja(player);
        bendingPlayer.abilities = offlineBendingPlayer.abilities;
        bendingPlayer.styles.addAll(offlineBendingPlayer.styles);
    ...
    protected static OfflineBendingPlayer convertToOffline(@NotNull Ninja bendingPlayer) {
        ...
        offlineBendingPlayer.styles.addAll(bendingPlayer.styles);
    ...
    /**
     * Uncaches this instance of an Offline Ninja.
     */
    public void uncache() {
        if (this.player.isOnline() || this instanceof Ninja) return;
