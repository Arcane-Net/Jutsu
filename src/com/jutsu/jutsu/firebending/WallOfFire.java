...
public class WallOfFire extends FireAbility {
	...
	public WallOfFire(final Player player) {
		...
		if (hasAbility(player, WallOfFire.class) && !this.nPlayer.isAvatarState()) {
			return;
		} else if (this.nPlayer.isOnCooldown(this)) {
			return;
		}
		...
		widthMod = (int) (nPlayer.canUseSubElement(SubElement.BLUE_FIRE) ? (BlueFireAbility.getRangeFactor() * width - width) + widthMod : widthMod);
		heightMod = (int) (nPlayer.canUseSubElement(SubElement.BLUE_FIRE) ? (BlueFireAbility.getRangeFactor() * height - height) + heightMod : heightMod);
		durationMod = (int) (nPlayer.canUseSubElement(SubElement.BLUE_FIRE) ? (duration / BlueFireAbility.getCooldownFactor() - duration) + durationMod : durationMod);
		damageMod = (int) (nPlayer.canUseSubElement(SubElement.BLUE_FIRE) ? (BlueFireAbility.getDamageFactor() * damage - damage) + damageMod : damageMod);
