...
public class FireBlast extends FireAbility {
	...
	public FireBlast(final Player player) {
		...
		if (this.nPlayer.isOnCooldown("FireBlast")) {
			return;
		...
		this.start();
		this.nPlayer.addCooldown("FireBlast", this.cooldown);
	}
	...
	public boolean checkLocation(Block block) {
		...
		if (!block.isPassable()) {
			if (block.getType() == Material.FURNACE && this.powerFurnace) {
				...
				if(!campfire.isLit()) {
					if(block.getType() != Material.SOUL_CAMPFIRE || nPlayer.canUseSubElement(SubElement.BLUE_FIRE)) {
	...
	public void progress() {
		if (!this.nPlayer.canBendIgnoreBindsCooldowns(this) || RegionProtection.isRegionProtected(this, this.location)) {
			this.remove();
			return;
		}
