...
public class FireBlastCharged extends FireAbility {
	...
	public FireBlastCharged(final Player player) {
		...
		if (!this.nPlayer.canBend(this) || hasAbility(player, FireBlastCharged.class)) {
			return;
		}
	...
	private void applyModifiers() {
		...
		chargeTimeMod = (long) (nPlayer.canUseSubElement(SubElement.BLUE_FIRE) ? (chargeTime / BlueFireAbility.getCooldownFactor() - chargeTime) + chargeTimeMod : chargeTimeMod);
		minDamageMod = (nPlayer.canUseSubElement(SubElement.BLUE_FIRE) ? (BlueFireAbility.getDamageFactor() * minDamage - minDamage) + minDamageMod : minDamageMod);
		maxDamageMod = (nPlayer.canUseSubElement(SubElement.BLUE_FIRE) ? (BlueFireAbility.getDamageFactor() * maxDamage - maxDamage) + maxDamageMod : maxDamageMod);
		rangeMod =  (nPlayer.canUseSubElement(SubElement.BLUE_FIRE) ? (BlueFireAbility.getRangeFactor() * range - range) + rangeMod : rangeMod);
	...
	public void progress() {
		if (!this.nPlayer.canBendIgnoreBinds(this) && !this.launched) {
			this.remove();
			return;
		} else if (!this.nPlayer.canBendIgnoreCooldowns(CoreAbility.getAbility("FireBlast")) && !this.launched) {
			this.remove();
			return;
