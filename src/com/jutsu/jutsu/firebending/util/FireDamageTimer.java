...
public class FireDamageTimer {
	...
	public static void dealFlameDamage(final Entity entity) {
		if (INSTANCES.containsKey(entity) && entity instanceof LivingEntity) {
			...
			// damages the entity.
			if (ability == null) {
				DamageHandler.damageEntity(Lentity, source, DAMAGE, CoreAbility.getAbilitiesByElement(Style.FIRE).get(0));
			} else {
	...
	/**
	 * Util so that players can find benders who were hurt/killed by jutsu
