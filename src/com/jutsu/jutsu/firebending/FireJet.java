...
public class FireJet extends FireAbility {
	...
	public void progress() {
		if (this.player.isDead() || !this.player.isOnline()) {
			this.remove();
			return;
		} else if ((isWater(this.player.getLocation().getBlock()) || System.currentTimeMillis() > this.time + this.duration) && (!this.nPlayer.isAvatarState() || !this.avatarStateToggled)) {
			this.remove();
			return;
		} else {
			...
			if (this.nPlayer.isAvatarState() && this.avatarStateToggled) {
				timefactor = 1;
			} else {
